...
namespace claid
{
    ...
    {
        Path storageLogPath = Path::join(storageFolderPath, "Data", "Logs");

        if (!FileUtils::dirExists(storageLogPath.toString()))
        {
            if (!FileUtils::createDirectoriesRecursively(storageLogPath.toString()))
            {
                return;
            }
        }

        if (storageLogPath.toString() == "" || !FileUtils::dirExists(storageLogPath.toString()))
        {
            return;
        }
        ...
        // Format for filename: moduleId_yy.mm.dd_events.txt
        std::string dateString = time.strftime("%y.%m.%d");
        Path path = Path::join(storageLogPath.toString(), moduleId + "_" + dateString + "_events.txt");
    ...
    void EventTracker::logGeneralEvent(const std::string &event)
    {
        ...
        // Format for filename: CLAID_yy.mm.dd_events.txt
        std::string dateString = time.strftime("%y.%m.%d");
        Path path = Path::join(storageLogPath.toString(), "CLAID_" + dateString + "_events.txt");
        std::ofstream file(path.toString(), std::ios::app);
